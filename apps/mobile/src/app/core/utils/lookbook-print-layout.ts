import {
  paginateLookbookTemplates,
  type LookbookTemplateShift,
} from './paginate-lookbook';

/** A4 at 96dpi — same numbers as Cloud PDF (Puppeteer viewport). */
export const LOOKBOOK_A4 = {
...
/**
 * Content layout CSS — must stay in sync with functions/src/print-template.ts
 * (`buildPrintContentCss`). Used for page measurement + live editor parity.
 */
export function buildLookbookPrintContentCss(bodySel = '.doc-body'): string {
  return `
    ${bodySel} {
    ...
    .lb-tpl {
      ...
      /* Match Cloud PDF continuous A4 banding (no avoid-push). */
...
/** Inject print layout into the live editor sheet so preview ≈ PDF. */
export function ensureEditorPrintParityCss(): void {
  if (typeof document === 'undefined') return;
  const existing = document.getElementById('lb-print-parity-css');
  if (existing) existing.remove();
  const style = document.createElement('style');
  style.id = 'lb-print-parity-css';
  const scope = '.lb-rich__sheet .ProseMirror, .lb-rich__sheet .lb-pm__mount .ProseMirror';
  style.textContent = `
.lb-rich__sheet {
  font-family: "Outfit", system-ui, sans-serif;
}
...
${buildLookbookPrintContentCss(scope)}
`;
  document.head.appendChild(style);
}

/** Print CSS mirroring functions/src/print-template.ts (px, not mm). */
export function buildLookbookPrintDocument(opts: {
  title: string;
  bodyHtml: string;
}): string {
  const title = escapeHtml(opts.title || 'Lookbook');
  const body = opts.bodyHtml || '<p></p>';
  const { widthPx, heightPx, padTopPx, padXPx, padBottomPx } = LOOKBOOK_A4;
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    ${buildLookbookPrintContentCss('.doc-body')}
  </style>
</head>
<body>
  <div class="sheet">
...
export interface LookbookPrintMeasure {
  pageCount: number;
  totalHeight: number;
  pageHeight: number;
  /** Per-template margin pushes so blocks do not straddle A4 bands. */
  templateShifts: LookbookTemplateShift[];
}

/**
 * Measure how many A4 pages Cloud PDF will produce for this HTML.
 * Uses a hidden iframe at the same width as Puppeteer.
 */
export async function measureLookbookPrintPages(opts: {
  title: string;
  bodyHtml: string;
}): Promise<LookbookPrintMeasure> {
  const pageHeight = LOOKBOOK_A4.heightPx;
  const pageWidth = LOOKBOOK_A4.widthPx;
  ...
  try {
    ...
    if (!doc) {
      return { pageCount: 1, totalHeight: pageHeight, pageHeight, templateShifts: [] };
    }
    doc.open();
    doc.write(buildLookbookPrintDocument(opts));
    doc.close();
    ...
    // One frame for layout after image decode.
    await new Promise<void>((r) => requestAnimationFrame(() => r()));

    // Same A4 template pagination as Cloud PDF (before height measure).
    const win = iframe.contentWindow;
    const templateShifts = paginateLookbookTemplates(
      doc,
      pageHeight,
      LOOKBOOK_A4.padTopPx,
      LOOKBOOK_A4.padBottomPx,
    );
    if (win) {
      // Force layout after margin pushes.
    ...
    return { pageCount, totalHeight, pageHeight, templateShifts };
  } finally {
    iframe.remove();
  }
}
