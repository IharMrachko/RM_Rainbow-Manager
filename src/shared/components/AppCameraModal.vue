...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const resizeAndDrawOverlay = () => {
      ...
      const cssW = isMobile.value ? 300 : rect.width;
      ...
      const thickness = isMobile.value ? 12 : 80; // для компа 80;
      const overSize = isMobile.value ? 0.4 : 2; // для компа 2
      const cx = cssW / 2;
      const cy = cssH / 2;
    ...
    const takePhotoWithFrameMasked = async () => {
      ...
      const zoom = isMobile.value ? 1.4 : 1.5; // пример scale
